...
def format_api_error(e: KimaiAPIError) -> str:
    ...
    if "should not contain extra fields" in details:
        ...
        text += (
            ...
            "time_project, ...). For a timesheet, 'billable' is only present when the "
            "token has edit_billable_own_timesheet or edit_billable_other_timesheet, "
            "and 'begin' / 'end' are absent in punch-in/out tracking mode unless the "
            "token has view_other_timesheet. On a user, 'avatar' exists only when the "
            "instance enables theme.avatar_url, which is off by default. On a customer, "
            "'address' exists only when that customer already has a legacy address; use "
            "addressLine1-3 / postCode / city. "
            "On action=update also 'teams' (bound on create only; use team_access) and "
            "'project' on a global activity. Retry without that field, or grant the "
            "permission."
        )
    if details:
        text += f"\nDetails: {details}"
